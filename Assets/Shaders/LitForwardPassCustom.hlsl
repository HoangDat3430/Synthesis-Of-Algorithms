...
Varyings LitPassVertex(Attributes input)
{
    ...
    output.uv.zw = TRANSFORM_TEX(input.texcoord, _MaskTex);
...
)
{
    ...
    float revealTop = step(mask.r, _Reveal + (_FadeOut * _Reveal));
    float revealBottom = step(mask.r, _Reveal - (_FadeOut * _Reveal));
    float revealDiff = revealTop - revealBottom;
    float3 finalColor = lerp(col, _ErodeColor, revealDiff);
    surfaceData.albedo = finalColor;
    surfaceData.alpha = col.a * revealTop;
